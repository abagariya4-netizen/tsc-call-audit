...
RUBRICS = {
    ...
    "ai_voice_bot": {
        "name": "AI Voice Bot Follow-Up",
        "intent": "OUTBOUND AI VOICE BOT calling leads who browsed Desk/Chair on the website. Goal: qualify the lead's need, share product/price/offer, push them to a TSC store visit. The bot must engage within the first 5 seconds before the customer hangs up, understand the customer's use case, name the right product, handle silence/hangup-attempts gracefully, and end with a concrete next step. This is a BOT, not a human — score on script execution quality and conversational software UX, not on tone/empathy.",
        "parameters": [
            ("Opening Hook", 10, "Did the bot say something compelling in the first ~5 seconds — naming the product the lead browsed (Desk or Chair) — before the customer hung up?"),
            ("Engagement Recovery", 10, "If the customer only said 'hello' or stayed silent, did the bot prompt again with a specific question rather than continuing the monologue?"),
            ("Need Identification", 15, "Did the bot ask use-case questions (work / gaming / study / hours per day) and actually parse the answer?"),
            ("Product Naming & Relevance", 15, "Did the bot mention a specific product (AeroPlus desk, Onyx chair, Flex Ergonomic chair — not just 'desk' or 'chair') matched to the use case?"),
            ("Price & Offer Communication", 10, "Did the bot share price AND current offer, clearly and concisely (no walls of text)?"),
            ("Store Visit Push", 15, "Did the bot mention the nearest store and explicitly invite a visit (with location and ideally timing)?"),
            ("Objection / Hesitation Handling", 10, "When the customer said 'later,' 'expensive,' or 'let me think' — did the bot respond with something specific, not generic?"),
            ("Language Adaptation", 5, "If the customer struggled with Hindi/English, did the bot acknowledge the barrier or switch register?"),
            ("Call Control & Pacing", 5, "Did the bot avoid talking over the customer, respect pauses, keep responses under ~10 seconds each?"),
            ("Closing & Next Step", 5, "Did the bot end with a concrete next action: store visit, callback time, SMS link, etc. — not just 'thank you'?"),
        ],
        "red_flags": [
            ("Bot continued monologue while customer was clearly hanging up", -15),
            ("Bot quoted wrong product or wrong price", -15),
            ("Bot couldn't handle 'not interested' — kept pitching", -10),
            ("Bot didn't acknowledge a language barrier (kept speaking Hindi/English when customer asked for Tamil/regional)", -10),
            ("Bot was robotic in a jarring way (mispronounced name, weird pauses, broken sentences)", -5),
            ("Customer asked a question bot couldn't answer, no fallback", -10),
            ("Bot didn't offer any next step at end of call", -10),
        ],
    },
}
